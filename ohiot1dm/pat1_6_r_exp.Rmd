...
lambda = 0.3
n_clusters_0_list = c(3,15,3,5,5,19)
n_clusters_1_list = c(2,8,2,3,3,5)
```
...
for (iter in 1:6) {
  ...
  for (nm in names(all_data)) {
    ...
    policy_vec =  apply(x$St, 1, FUN = function(x) policies[[tttt]](x))
...
```{r}
result_matrix <- as.matrix(read.csv("./OPE_Results/result_pat_First_Half.csv",header = TRUE))
...
```{r}
ggsave("ohiodm_ope_plot_cohort1.png", plot = p, width = 6, height = 4, dpi = 300)
```
